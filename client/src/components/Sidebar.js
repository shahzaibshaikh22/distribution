...
import { links,distribution,inventoryLinks,paymentsLink,productLinks,purchaseLinks,settings,setupLinks,stockLinks } from '../constraints/constraints';
...
const Sidebar = () => {
  ...
  const handleDropdownToggle = (menuText) => {
    setOpenDropdown((prev) => (prev === menuText ? null : menuText)); // 👈 Close others
  };

  return (
    <div className={`fixed top-0 left-0 z-20  ${sideBarCollapes === "show" ? 'w-[189px]' : 'w-[80px]'} min-h-screen ${modes === "dark" ? 'bg-darksecondary' : 'bg-white'}  md:block hidden transition-all duration-150 ease-linear`}>
...
//                 ${modes === "dark"
//                   ? location.pathname === link.path
//                     ? "bg-grayPrimary text-white"
//                     : "text-white hover:bg-darkprimary"
//                   : location.pathname === link.path
//                     ? "bg-lightsecondary text-black"
...
//           {/* Dropdown Menus */}
//           {/* Settings Dropdown */}
//           <div className={`group relative ${sideBarCollapes === "show" ? '' : 'flex justify-center'}`}>
//             <DropdownMenu
//               modes={modes}
//               links={settings}
//               menuText="Settings"
...
//               <div className="absolute left-[80px] w-48 bg-white shadow-lg rounded-md p-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
//                 {settings.map((item) => (
//                   <Link key={item.id} to={item.path} className="block px-4 py-2 text-sm text-black hover:bg-gray-200">
//                     {item.text}
//                   </Link>
//                 ))}
//               </div>
