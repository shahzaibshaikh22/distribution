...
import { setOrderReturn, setPurchaseReturn } from '../../redux/features/slices/productSlice';
import TopBar from '../../components/TopBar';
import { FaTrashAlt, FaEye } from 'react-icons/fa';
import { useDeleteOrderReturnMutation, useGetOrderReturnQuery } from '../../redux/features/apiSlices/booking/orderBooking';

const OrderReturnView = () => {
    const dispatch = useDispatch()
    const { orderReturn } = useSelector((state) => state.product);
    const [selectedReturn, setSelectedReturn] = useState(null)

    const { data: orderReturnData,refetch } = useGetOrderReturnQuery()
    ...
    useEffect(() => {
        if (orderReturnData) {
            dispatch(setOrderReturn(orderReturnData))
        }
    }, [orderReturnData, dispatch])
    const [deleteOrderReturn] = useDeleteOrderReturnMutation()

    const handleDelete = async (id) => {
        const res = await deleteOrderReturn(id);
        if (res.data.msg) {
          alert(res.data.msg);
          refetch();
        }
        ...
        <div className="w-full px-4">
            ...
            <div className="w-full bg-white rounded-md my-4 p-6">
                ...
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody>
                        {orderReturn.length > 0 && orderReturn?.map((r, index) => {
                            return (
                                <tr key={r._id} className="border-b p-1 hover:bg-lightsecondary">
                                    <td className="py-3 text-xs px-4">{index + 1}</td>
                                    <td className="py-3 text-xs px-4">{r.customer.name}</td>
                                    <td className="py-3 text-xs px-4">{r.customer.phone}</td>
                                    <td className="py-3 text-xs px-4">{r.customer.email}</td>
                                    <td className="py-3 text-xs px-4">{r.customer.address}</td>
                                    <td className="py-3 text-xs px-4">{r.bono}</td>
                                    ...
                                    <td className=" flex items-center gap-2 px-4 py-2">
                                        ...
                                        <button onClick={() => handleDelete(r._id.toString())} className="text-white bg-red-600 px-3 py-2 text-xs rounded-md">
                                            <FaTrashAlt />
                                        </button>
                                    </td>
                                </tr>
                    ...
                    <div className="flex gap-2 bg-white w-full md:max-w-5xl  rounded-md">
                        <div className="bg-white p-6 rounded-lg w-full  border-r">
                            ...
                            <table className="w-full border-collapse border border-gray-300 shadow-md">
                                <tbody>
                                    <tr className="bg-gray-100">
                                        <td className="border p-3 font-semibold text-gray-700">Order Booking NO</td>
                                        <td className="border p-3">{selectedReturn.bono}</td>
                                    </tr>
                                    <tr>
                                        <td className="border p-3 font-semibold text-gray-700">Customer Name</td>
                                        <td className="border p-3">{selectedReturn.customer.name}</td>
                                    </tr>
                                    <tr>
                                        <td className="border p-3 font-semibold text-gray-700">Phone</td>
                                        <td className="border p-3">{selectedReturn.customer.phone}</td>
                                    </tr>
                                    <tr className="bg-gray-100">
