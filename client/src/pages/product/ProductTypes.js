...
import { useAddProductTypeMutation } from "../../redux/features/apiSlices/product/productTypeApiSlice";
...
const handleSubmit = async (e)=>{
    ...
    const res = await addproductType({producttype})
    setProductType("")
    if(res){
        if(res.data.err){
            alert(res.data.err)
    ...
    return (
        <div className="w-full flex  text-white px-4">
            ...
            <div className="w-full ">
                <TopBar />
                <SectionBar sectionHeading="Product Type" />
                {/* form section */}
                <div className={`w-full md:px-10 mainContainerForm relative rounded-xl ${modes === "dark" ? 'bg-darksecondary text-white' : 'bg-white text-gray-800'} h-[4rem]`}>
                    <div className='md:px-0 px-10'>
                        <h1 className='text-center md:text-md text-sm pt-6 font-semibold mb-5'>New Complain</h1>
                ...
                <div className={`w-full Container h-auto md:mb-0 mb-4  px-10 ${modes === "dark" ? 'bg-darksecondary text-white' : 'bg-white text-gray-800'}`}>
                    ...
                    <form onSubmit={handleSubmit} className=' py-4'>
                        <div className='flex md:flex-row w-full md:max-w-[45%] flex-col md:gap-20  md:my-4 '>
                            ...
                            <InputField
                                value={producttype}
                                onChangeFunction={(e)=>setProductType(e.target.value)}
