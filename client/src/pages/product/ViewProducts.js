...
const ViewProducts = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    for (const key in editProduct) {
        console.log(key, editProduct[key]); // Check karo kya mil raha hai
        formData.append(key, editProduct[key]);
    }

    console.log("Final FormData:");
    for (let pair of formData.entries()) {
        console.log(pair[0], pair[1]); // Yeh check karega kya ja raha hai
    }

    const res = await updateproduct({formData,id:editProduct._id});
    if (res.data.msg) {
        setIsUpdate(false);
    }
};
    ...
    <>
      ...
      <div className="p-6 bg-white shadow rounded-lg">
        <div className="flex justify-between mb-4 flex-wrap">
          ...
          <Link to="/add-product" className="bg-blue-500 text-white px-4 py-2 rounded mt-4 sm:mt-0">Add Product</Link>
        </div>
